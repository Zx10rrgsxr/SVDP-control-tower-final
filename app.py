...
uploaded = st.sidebar.file_uploader('Upload workbook (optional)', type=['xlsx'])

from pathlib import Path
default_workbook = Path("svdp_final.xlsx")
...
# Priority 2: Default file in repo (AUTO LOAD)
...
if file_bytes is None:
    st.error("No workbook found. Please upload an Excel file.")
    st.stop()

st.success(f"Using workbook: {workbook_label}")
